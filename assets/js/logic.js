$(document).ready(function(){
    ...
    // food options
    var foodList = [lightWhite = {0:"tuna",1:"trout",2:"kale",3:"chickpea"},sweetWhite = {0:"proscuitto",1:"habanero",2:"strawberry",3:"sweet+potato"}, richWhite = {0:"duck", 1:"lobster",2:"mushroom",3:"curry"},sparkling = {0:"oyster",1:"mussels",2:"clam",3:"pecorino"},rose={0:"turnip",1:"butternut",2:"pumpkin",3:"salami"},lightRed = {0:"chicken",1:"mushroom",2:"duck",3:"bacon"},mediumRed = {0:"pork",1:"gorgonzola",2:"eggplant",3:"tomato"},boldRed = {0:"beef",1:"lamb",2:"steak",3:"cheddar+cheese"}
    ];
    var type = "";
    var ingredient = "";
    var wineType = "";
    ...
    $(".wines").on("click",function(event){
        ...
        $("#searchbutton").on("click",function(){
            ...
            $.ajax({
                ...
                $("#f0").attr("src","https://www.yummly.com/recipe/"+response.matches[0].id)
            ...
            $.ajax({
                ...
                $("#f1").attr("src","https://www.yummly.com/recipe/"+response.matches[0].id)
                console.log(response.matches[0].id)
                $("#zoomin").show()
            }); 
            $.ajax({
                ...
                $("#f2").attr("src","https://www.yummly.com/recipe/"+response.matches[0].id)
                console.log(response.matches[0].id)
                $("#zoomin").show()
            }); 
            $.ajax({
                ...
                $("#f3").attr("src","https://www.yummly.com/recipe/"+response.matches[0].id)
                console.log(response.matches[0].id)
                $("#zoomin").show()
            }); 
            // };
            $.ajax({
                url: "http://api.snooth.com/wines/?akey=k53smv0z7gy01bnritao0gd790090xjeum6xllqjsm8bu8yd&q="+wineType+"&xp=40",
                method: "GET"
            }).then(function(response){
                
                // $("#wine-table").append("<tr><th>Label</th><th>Wine</th><th>Vintage</th><th>Price</th><th>Link</th></tr>");
                // for (i=0;i<4;i++){
                // console.log(response.wines);    
                //     // $("#wine-table").append("<tr><td><img src='"+response.wines[i].image+"'></td><td>"+response.wines[i].name+"</td><td>"+response.wines[i].vintage+"</td><td>"+response.wines[i].price+"</td><td>"+response.wines[i].link+"</td></tr>");
                // }
            });
